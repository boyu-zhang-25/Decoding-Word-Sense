...
def get_all_hiddens(path):
	...
	# for idx in range(small_size):
		...
		# mem saving, only inference
		with torch.no_grad():

			# sense embedding from the graph_lstm runing on the target synset node
			# hyper_hypon_graph
			hyper_hypon_h_all, (hyper_hypon_hidden, hyper_hypon_cell) = hyper_hypon_graph(synset, depth = 4)

			# mer_holo_graph
			mer_holo_h_all, (mer_holo_hidden, mer_holo_cell) = mer_holo_graph(synset, depth = 4)
		...
		if idx % 100 == 0:
			print("[{}/{}] done synsets.".format(idx, synset_vocab_SemCor.idx))
