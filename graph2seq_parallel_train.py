        ...
        for sent in sub_corpus:
            ...
            if len(tagged_sent) > 0:
                ...
                for instance in tagged_sent:
                    ...
                    synset, definition = get_SemCor_def(instance)
                ...
                output, _ = model(senses, tagged_sent, definitions, teacher_forcing_ratio = 0.4)
            ...
            for sent in sub_corpus:
                ...
                if len(tagged_sent) > 0:
                    ...
                    for instance in tagged_sent:
                        ...
                        # get the sense from the target file with ID
                        synset, definition = get_SemEval_def(instance)
                        def_idx_list = def2idx(definition, model.max_length, vocab)
                        definitions.append(def_idx_list)
                    ...
                    # get the encoder-decoder result
                    # (self.max_length, batch_size, vocab_size)
                    output, result = model(senses, tagged_sent, definitions, teacher_forcing_ratio = 0)
...
for epoch in range(N_EPOCHS):
    ...
    train_loss = train(graph2seq_model, optimizer, semcor_corpus, criterion, CLIP)
    train_losses.append(train_loss)
    
    valid_loss, all_sentence_result, all_definitions = evaluate(graph2seq_model, semeval_corpus, criterion)
